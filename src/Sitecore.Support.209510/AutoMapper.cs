namespace Sitecore.Support.Forms.Mvc.Services
{
  ...
  {
    private HtmlString RenderField(string fieldName, Item item)
    {
      Assert.ArgumentNotNull(fieldName, "fieldName");
      RenderFieldArgs innerWrappedObject = new RenderFieldArgs
      {
        Item = item,
        FieldName = fieldName
      };

      CorePipeline.Run("renderField", innerWrappedObject);
      RenderFieldResult source = innerWrappedObject.Result;
      string str = source.ValueOrDefault<RenderFieldResult, string>(result => result.FirstPart).OrEmpty();
      string str2 = source.ValueOrDefault<RenderFieldResult, string>(result => result.LastPart).OrEmpty();
    ...
    public FormViewModel GetView(IFormModel formModel)
    {
      ...
      var formViewModel = new FormViewModel
      {
        ...
        Information = (!String.IsNullOrEmpty(formModel.Item.Introduction)) ? RenderField("Introduction", formModel.Item.InnerItem).ToHtmlString() : string.Empty,
        ...
        Footer = (!String.IsNullOrEmpty(formModel.Item.Footer)) ? RenderField("Footer", formModel.Item.InnerItem).ToHtmlString() : String.Empty,
